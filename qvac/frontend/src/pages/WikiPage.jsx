...
export default function WikiPage({ onBack }) {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`${API_BASE}/llmwiki-save`, {
          ...
          body: JSON.stringify({ content: editorText, title, category: saveCategory, id: selectedDoc || undefined })
        });
        ...
        if (json.success) {
          ...
          if (json.data?.id && !selectedDoc) setSelectedDoc(json.data.id);
          await fetchDocs();
        } else {
  ...
  }, [editorText, saveTitle, saveCategory, selectedDoc]);
  ...
  const startNode = async () => {
    if (!authenticated || !user?.wallet?.address) {
      setSaveStatus('Please log in and create a wallet first');
  ...
  const stopNode = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/stop`, { method: 'POST', headers: { 'Content-Type': 'application/json' } });
      const text = await res.text();
      const json = text ? JSON.parse(text) : {};
      if (json.success) {
